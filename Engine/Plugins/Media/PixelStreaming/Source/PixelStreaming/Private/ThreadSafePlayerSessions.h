...
class FThreadSafePlayerSessions : public IPixelStreamingSessions
{
    public:
		...
		void SendData(const TArray<uint8>& ByteData, const FString& MimeType, const FString& FileExtension);
	...
	private:
		...
		void SendData_SignallingThread(const TArray<uint8>& ByteData, const FString& MimeType, const FString& FileExtension);
