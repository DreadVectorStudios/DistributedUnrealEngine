...
{
	SendArbitraryData(DataChannel, static_cast<TArray<uint8>>(JpegBytes), static_cast<uint8>(PixelStreamingProtocol::EToPlayerMsg::FreezeFrame));
}

void FPlayerSession::SendData(const TArray<uint8>& FileBytes, const FString& MimeType, const FString& FileExtension) const
{
	...
	AsyncTask(ENamedThreads::AnyHiPriThreadHiPriTask, [&, FileBytes]()
	{
		SendArbitraryData(this->DataChannel, FileBytes, static_cast<uint8>(PixelStreamingProtocol::EToPlayerMsg::FileContents));
	});
}
