...
void FThreadSafePlayerSessions::SendData(const TArray<uint8>& ByteData, const FString& MimeType, const FString& FileExtension)
{
    SUBMIT_TASK_WITH_PARAMS(SendData_SignallingThread, ByteData, MimeType, FileExtension)
}
...
void FThreadSafePlayerSessions::SendData_SignallingThread(const TArray<uint8>& ByteData, const FString& MimeType, const FString& FileExtension)
{
    checkf(this->IsInSignallingThread(), TEXT("This method must be called on the signalling thread."));
	...
	UE_LOG(PixelStreamer, Log, TEXT("Sending file with Mime type %s to all players: %d bytes"), *MimeType, ByteData.Num());
    {
        for(auto& PlayerEntry : this->Players)
        {
            PlayerEntry.Value->SendData(ByteData, MimeType, FileExtension);
        }
    }
}
