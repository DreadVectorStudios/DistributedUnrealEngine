...
//		checkSlow(VertexFactoryData.MorphVertexFactories.Num() == 0);
	...
	if (bDataPresent)
	{
		for (int32 SectionIdx = 0; SectionIdx < Sections.Num(); SectionIdx++)
		{
			...
			const bool bClothFactory = RHISupportsManualVertexFetch(GMaxRHIShaderPlatform) && (FeatureLevel >= ERHIFeatureLevel::SM5) &&
								 SimData && Section.HasClothingData();
			...
			if (bUseSkinCache)
			{
				FMatrix ClothToLocal = bClothFactory ? VertexFactoryData.ClothVertexFactories[SectionIdx]->GetClothShaderData().GetClothToLocalForWriting(FrameNumberToPrepare) : FMatrix::Identity;
