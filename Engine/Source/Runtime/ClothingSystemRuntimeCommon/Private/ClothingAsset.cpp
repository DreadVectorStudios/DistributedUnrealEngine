...
void UClothingAssetCommon::PostLoad()
{
	...
	// Check for missing tether data (Note, this is a hack for 4.27-Chaos to avoid breaking the serialization)
	for (FClothLODDataCommon& Lod : LodData)
	{
		const FClothPhysicalMeshData& PhysMesh = Lod.PhysicalMeshData;

		if (!PhysMesh.EuclideanTethers.Tethers.Num() &&
			!PhysMesh.GeodesicTethers.Tethers.Num())
		{
			// No tethers, this is suspicious, better check the max distance map to see if the cached data is missing
			const FPointWeightMap* const MaxDistances = PhysMesh.FindWeightMap(EWeightMapTargetCommon::MaxDistance);
			if (MaxDistances && MaxDistances->Num())
			{
				const bool BelowThreshold = MaxDistances->IsBelowThreshold(0);
				for (int32 Index = 1; Index < MaxDistances->Num(); ++Index)
				{
					if (MaxDistances->IsBelowThreshold(Index) != BelowThreshold)
					{
						// There should have been tethers on this asset, but they aren't cached yet
		...
		if (EnumHasAllFlags(ClothingCachedDataFlags, EClothingCachedDataFlagsCommon::Tethers))
		{
			break;  // Early exit at the first LOD requiring cached tethers
		}
	}
